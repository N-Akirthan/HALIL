...
export function ProductCard({ product, onQuickView }: Props) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="block h-full">
      <div className="group bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 hover:border-amber-355 dark:hover:border-amber-500 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between items-center w-full h-full relative overflow-hidden text-center">
        
        {/* Catégorie */}
        <span className="text-[10px] tracking-widest text-[#006680] dark:text-amber-400 font-black uppercase mb-3 px-2.5 py-0.5 rounded-full bg-slate-50 dark:bg-slate-800 select-none">
        ...
        {/* Titre produit */}
        <h2 className="text-slate-800 dark:text-slate-100 mt-4 text-xs xl:text-sm font-extrabold text-center leading-tight uppercase px-1 min-h-[2.5rem] group-hover:text-[#006680] dark:group-hover:text-amber-400 transition-colors">
          {product.name.split("\n").map((line, index) => (
            <Fragment key={index}>
              {line}
              <br />
        ...
        {/* Découvrir */}
        <div className="mt-3 text-[10px] font-black text-slate-400 dark:text-slate-500 group-hover:text-[#ebc834] dark:group-hover:text-amber-400 transition-colors uppercase tracking-widest flex items-center gap-1">
